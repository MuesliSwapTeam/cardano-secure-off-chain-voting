...
const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      let minAmount = Loader.Cardano.Value.new(Loader.Cardano.min_ada_for_output(transactionOutput, dataCost))
...
function descSelect(utxoSelection, outputAmount, limit, minUTxOValue) {
  ...
  utxoSelection.subset = utxoSelection.subset.sort((a, b) => {
    return Number(
      searchAmountValue(outputAmount, b.output().amount()) - searchAmountValue(outputAmount, a.output().amount()),
    )
  })
