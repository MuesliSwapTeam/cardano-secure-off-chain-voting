import { ORDERBOOK_PLUTUS_V1_ADDRESS, ORDERBOOK_PLUTUS_V1_CONTRACT, ORDERBOOK_REVENUE_ADDRESS } from 'cardano/constants'
...
import { getCancelPlutusV1Redeemer, getOrderbookDatum } from 'cardano/muesliswap/shared'
...
export async function cancelOrder(
  ...
  const plutusWitness = Loader.Cardano.PlutusWitness.new(ORDERBOOK_PLUTUS_V1_CONTRACT(), datum, redeemer)
...
export async function placeOrder(
  ...
  var output = createOutput(ORDERBOOK_PLUTUS_V1_ADDRESS(), assetsToValue(assets), datum, hash)
  outputs.add(output)
  ...
  const txHash = await finalizeTX(wallet, txBuilder, walletAddress, utxos, outputs, datums, newMetadata, null, null)
